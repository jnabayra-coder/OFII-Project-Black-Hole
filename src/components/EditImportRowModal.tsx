...
import { getAutoDeliveryLeadTime, computeDeliveryPerformance, computePodPerformance } from '../utils/forwardingCalculations';
...
export const EditImportRowModal: React.FC<EditImportRowModalProps> = ({
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    if (dispatchDate && actualDeliveryDate) {
      const p = computeDeliveryPerformance(dispatchDate, actualDeliveryDate, leadTime);
      delTat = p.tatDays;
      delPerf = p.performance;
    }

    let podTat = 0;
    let podPerf = formData.podPerformance;
    if (actualDeliveryDate && dateOfPodReturn) {
      const p = computePodPerformance(actualDeliveryDate, dateOfPodReturn, 3);
      podTat = p.podTatDays;
      podPerf = p.podPerformance;
    }
    ...
    const updated: ForwardingProgressiveRecord = {
      ...
      deliveryStatus: actualDeliveryDate ? 'Delivered' : formData.deliveryStatus,
